...
class BBVAStatementStrategy implements StrategyInterface
{
    ...
    private function parseStatement($content): Statement
    {
        ...
        $monthDetermination = 'start-date';
        $monthDetermination = 'most-days';
        if ($monthDetermination === 'start-date') {

            $startDate = $matches[1];
            // $endDate   = $matches[2];
            [, $month, $year] = explode('/', $startDate);
        } else {
            // Alternative determination of month/year based on which month overlaps more with the statement's period.
